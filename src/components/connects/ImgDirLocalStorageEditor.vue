...
const connect: Ref<ImgDirLocalStorageConnect> = ref((props.mode === 'add' ? connects.new('imgdirls') : connects.clone(connectid)) as ImgDirLocalStorageConnect);
const changed = computed<boolean>(() => {
  return (
    props.mode === 'add' ?
      (!connects.has(connect.value.id))
      :
      (connect.value.extends.path != (connects.get(connect.value.id) as ImgDirLocalStorageConnect).extends.path)
  );
})
...
function save() {
  switch (props.mode) {
    case 'add': connects.add(connect.value); break;
    case 'edit': connects.update(connect.value); break;
  }
};

async function collect() {
  connect.value.state.collect = 'collecting';
  connects.collectImgDirLocalStorageConnect(connect.value).then((count: number) => {
